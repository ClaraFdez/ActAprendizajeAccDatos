...
public class AutobusController {
    ...
    public ResponseEntity<List<Autobus>> findAllBusSec(@PathVariable long sector_id){
    ...
    public ResponseEntity<Autobus> addBus(@RequestBody Autobus bus){
        logger.info("Inicio de addBus");
        Autobus busadd = autobusService.addBus(bus);
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Modificado con exito", content = @Content(schema = @Schema(implementation = Autobus.class))),
    ...
    public ResponseEntity<Autobus> modifyBus(@PathVariable long idBus, @RequestBody Autobus newbus){
        logger.info("Inicio de modifyBus");
        Autobus bus;
        bus = autobusService.modifyBus(idBus, newbus);
    ...
    public ResponseEntity<Respuesta> deleteBus(@PathVariable long idBus){
        logger.info("Inicio de deleteBus");
        autobusService.deleteBus(idBus);
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Modiicado con exito", content = @Content(schema = @Schema(implementation = Autobus.class))),
            @ApiResponse(responseCode = "404", description = "El autobus no existe", content = @Content(schema = @Schema(implementation = Respuesta.class)))
    })
    @PatchMapping(value = "/bus", produces = "application/json", consumes = "application/json")
    public ResponseEntity<Autobus> modifyBusPrecio(@PathVariable long idBus, @RequestParam(name = "precio") float precio){
        logger.info("Inicio modifyBusPrecio");
        Autobus bus;
        bus = autobusService.modifyBusPrecio(idBus, precio);
    ...
    @GetMapping(value = "/bus/{id}", produces = "application/json", consumes = "application/json")
    public ResponseEntity<Autobus> findById(@PathVariable long idBus){
        logger.info("Inicio de findById");
        Autobus autobus;
        autobus = autobusService.findById(idBus);
